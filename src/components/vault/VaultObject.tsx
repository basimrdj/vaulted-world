...
function Card3D({ index, burstProgress, arrangeProgress, vaultX, totalCards }: Card3DProps) {
  const meshRef = useRef<THREE.Mesh>(null);
  const currentPos = useRef(new THREE.Vector3(0, 0, 0));
  const currentRot = useRef(new THREE.Euler(0, 0, 0));

  // Burst trajectories — explosive outward from vault center
  const burstTarget = useMemo(() => {
    const angle = (index / totalCards) * Math.PI * 2 + Math.PI * 0.2;
    const radius = 2.5 + (index % 3) * 0.8;
    return new THREE.Vector3(
      Math.cos(angle) * radius,
      Math.sin(angle) * radius * 0.6 + (index % 2 === 0 ? 0.5 : -0.3),
      1.5 + Math.random() * 2
    );
  }, [index, totalCards]);

  // Final arranged grid positions
  const arrangedTarget = useMemo(() => {
    ...
    const spacing = 2.2;
    return new THREE.Vector3(
      (col - 1) * spacing + 1.5, // offset right since vault moves left
      (0.5 - row) * 2.8,
      3.5
    );
  }, [index]);
  ...
  useFrame(() => {
    ...
    if (arrangeProgress <= 0) {
      // Bursting out — interpolate from origin to burst position
      const eased = easeOutBack(burstProgress);
      targetPos = origin.clone().lerp(burstTarget, eased);
      targetRotX = burstProgress * (index % 2 === 0 ? 1 : -1) * 0.5;
      targetRotY = burstProgress * Math.PI * 0.3;
      targetRotZ = burstProgress * (index % 2 === 0 ? 0.2 : -0.2);
    } else {
      // Arranging — interpolate from burst position to grid
      const eased = easeOutCubic(arrangeProgress);
      targetPos = burstTarget.clone().lerp(arrangedTarget, eased);
      targetRotX = (1 - arrangeProgress) * 0.5 * (index % 2 === 0 ? 1 : -1);
      targetRotY = (1 - arrangeProgress) * Math.PI * 0.3;
      targetRotZ = 0;
    }

    // Damped spring motion
    currentPos.current.lerp(targetPos, 0.08);
    currentRot.current.x += (targetRotX - currentRot.current.x) * 0.06;
    currentRot.current.y += (targetRotY - currentRot.current.y) * 0.06;
    currentRot.current.z += (targetRotZ - currentRot.current.z) * 0.06;
  ...
  return (
    <mesh ref={meshRef} visible={false}>
      <roundedBoxGeometry args={[1.6, 2.2, 0.06, 4, 0.08]} />
      <meshPhysicalMaterial
        color={card.color}
        metalness={0.2}
        roughness={0.1}
