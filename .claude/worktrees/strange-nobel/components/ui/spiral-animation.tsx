...
class AnimationController {
  ...
  public spiralPath(p: number): Vector2D {
    let t = this.constrain(1.2 * p, 0, 1)
    t = this.ease(t, 1.8)
    const numberOfSpiralTurns = 6
    const theta = 2 * Math.PI * numberOfSpiralTurns * Math.sqrt(t)
...
class Star {
  ...
  render(p: number, controller: AnimationController) {
    ...
    if (q > 0) {
      ...
      const vx = ((this as any).z - (controller as any)['cameraZ']) * screenX / (controller as any)['viewZoom']
      const vy = ((this as any).z - (controller as any)['cameraZ']) * screenY / (controller as any)['viewZoom']
